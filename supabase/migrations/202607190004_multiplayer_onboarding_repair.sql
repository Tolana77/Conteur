-- Répare une base où les RPC d'accueil n'ont pas été exposés par PostgREST.
-- Cette migration reste idempotente et peut être appliquée après 001, 002 et 003.
...
begin
  ...
  on conflict (room_id, user_id) do update
    set display_name = excluded.display_name,
        ...
        character_id = case
          when excluded.role = 'spectator'
            and public.multiplayer_members.role not in ('host', 'admin')
            then null
...
begin
  ...
  if jsonb_typeof(p_character_package) <> 'object'
    or pg_column_size(p_character_package) > 262144 then
...
begin
  ...
  if not found or v_member.role not in ('player', 'admin')
    or v_member.character_id is not null then
  ...
  if jsonb_typeof(v_package) <> 'object'
    or pg_column_size(v_package) > 262144 then
    raise exception 'INVALID_CHARACTER_PACKAGE';
  end if;
...
-- L'admin choisit les attributions. L'hôte conserve ce droit uniquement pour
-- finaliser techniquement les demandes installées dans son moteur local.
